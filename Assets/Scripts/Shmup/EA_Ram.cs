...
public class EA_Ram : IEnemyAbility
{
    ...
    public EA_Ram(Enemy enemy, float timeToCharge, float speed, float timeToRecover, float distance, Transform player){
        currentEnemy = enemy;
    ...
    public void UseEnemyAbility(){
        if(target == Vector3.zero){
            ...
            if(Vector3.Distance(targetplayer.position, currentEnemy.transform.position) < 0.2f){
                Debug.Log("ARRIVED!");
    ...
    public void Aim(){
        if(Time.time <= chargeTime){
            ...
            //Debug.Log("target hitting: " + target + " magnitude:" + target.magnitude);
            targetplayer.position = targetplayer.TransformDirection(target) + currentEnemy.transform.position; //offset by the object relative to it
