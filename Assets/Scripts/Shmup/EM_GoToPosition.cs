...
public class EM_GoToPosition : IMovementOption
{
    ...
    public EM_GoToPosition(EnemyController enemy, Transform target, Transform playerRef, bool setRandom,float initialSpeed, bool rotation = false){
    ...
    //only gets called on target decision
    private void OutOfBoundsY(){
        if(currentEnemy.transform.position.y >= 7f){
            ...
            currentTargetPosition = new Vector3(currentEnemy.transform.position.x,7f,0f);
    ...
    public void FindNewTarget(Vector3 dir){
        ...
        float selectionRanges = 0f; // consider changing to 90 if the situation calls for it! 
        float distanceTravel = 8f;
